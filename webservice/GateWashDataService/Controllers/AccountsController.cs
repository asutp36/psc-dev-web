...
namespace GateWashDataService.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        public AccountsController(ILogger<AccountsController> logger, GateWashDbContext context)
        {
            _logger = logger;
            _context = context;
        }
        ...
        {
            User user = _context.Users.FirstOrDefault((System.Linq.Expressions.Expression<Func<User, bool>>)(u => EF.Functions.Like(u.Login, login.login) && EF.Functions.Like(u.Password, login.password)));
            if (user != null)
            {
                ...
                {
                    string washCode = _context.Washes.Find(w.Idwash).Code;
        ...
        public IActionResult Register(AccountRequestModel account)
        {
            try
            {
                ...
                if (_context.Users.Where(u => u.Login == account.login).FirstOrDefault() != null)
                ...
                SqlHelper.WriteUser(account, _context);
            ...
            try
            {
                ...
                foreach (User u in users)
                {
                    ...
                    foreach (UserWash w in washIDs)
                        washes.Add(_context.Washes.Find(w.Idwash).Code);
        ...
        public IActionResult Delete(string login)
        {
            try
            {
                ...
                SqlHelper.DeleteUser(login, _context);
        ...
        public IActionResult Update(UpdateAccountRequestModel account)
        {
            try
            {
                ...
                if (_context.Users.Where(u => u.Login == account.oldLogin).FirstOrDefault() == null)
                {
                    return NotFound(new Error() { errorType = "badvalue", alert = "Пользователя с таким логином не существует", errorCode = "Пользователь не найден", errorMessage = "Проверьте, правильно ли введён логин пользователя, и попробуйте снова" });
                }

                SqlHelper.UpdateUser(account, _context);
        ...
        {
            AccountViewModel result = _context.Users.Where(u => u.Login == login).Include(u => u.IdroleNavigation)
